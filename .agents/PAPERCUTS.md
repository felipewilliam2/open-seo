...
- [ ] `2026-07-27T21:55:00Z` — `claude` — The resource-id comment in `wrangler.jsonc` ("All Cloudflare deployments — previews, prod, self-host — go through Alchemy … never reads these ids … Leave them as-is") contradicts `docs/SELF_HOSTING_CLOUDFLARE_MANUAL.md`, which tells self-hosters to replace those exact ids and run `wrangler deploy`. A self-hoster who trusts the comment leaves upstream's ids in place and then has `wrangler deploy` and `db:migrate:prod` (which resolves the `DB` binding from this file) point at an account they don't own. Name the self-host exception in the comment.
